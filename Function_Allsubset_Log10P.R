############ 不同特征个数的Best性能计算
# 存储5*8个模型描述符选择情况
Function_Allsubset_Log10P = function(){
	...
	library(caTools)
	set.seed(22)
	TM=sample.split(train_data$Log10P,4/5) #不能对矩阵用$提取对应名称的列
	...
	df$Intercept = NA
	# Intercept的显著性：summary(LM)$coefficients[1,4]
	df$coef1 = NA
	df$Parameter1 = NA
	df$coef2 = NA
	df$Parameter2 = NA
	...
	for (i in Model){
		...
		LM = lm(Log10P~.,data=train_data)
		
		obsTrain=10^train_data$Log10P
		predTrain=10^predict(LM,train_data)
		
		obsTest=10^test_data$Log10P
	...
	write.csv(df,
			file=paste("0 ML_Steps_2nearZeroVar_10P_005_resub_",
			length(Model),".csv",sep = ""),row.names=F)
	print(paste("##### 0 ML_Steps_2nearZeroVar_10P_005_resub_",length(Model),".csv Done",sep = ""))
